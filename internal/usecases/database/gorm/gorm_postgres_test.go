...
func TestMain(m *testing.M) {
	...
	connection, err := injections.BuildConnectionGorm(settings, logger, txKey)
	if err != nil {
		logger.Fatal(err.Error())
	}
	conn = connection
	...
	storages, err := injections.BuildStoragesGorm(connection)
	if err != nil {
		logger.Error(err.Error())
	}
	ds, es, ps = storages.Ds, storages.Es, storages.Ps

	gormMigrationTool, err := injections.BuildGooseMigrator(source.Migrations, settings.Type, connection)
	if err != nil {
		logger.Error(err.Error())
		return
	}
	...
	if err = gormMigrationTool.Up(folder); err != nil {
		logger.Error(err.Error())
		return
	}

	defer func() {
		if err = gormMigrationTool.DownAll(folder); err != nil {
			logger.Error(err.Error())
			return
		}
	}()
